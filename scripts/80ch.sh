...
# For local development, use merge-base with the upstream main branch
if [ -n "${GITHUB_ACTIONS:-}" ]; then
  ...
  if ! git rev-parse --verify main@{upstream} >/dev/null 2>&1; then
    echo "Error: Cannot find the upstream main branch." >&2
    echo "Please set up the origin remote:" >&2
    echo "  git remote add origin https://github.com/oxcaml/MocksCaml.git" >&2
